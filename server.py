...
def init_db():
    ...
    try: conn.execute('ALTER TABLE inventario_stock ADD COLUMN precio_venta REAL DEFAULT 0')
    except: pass
    conn.commit()
    conn.close()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/compras':
            ...
            cur = conn.execute(
                '''INSERT INTO inventario_stock (marca,modelo,chasis,motor,matricula,padron,precio,tipo,anio,color,compra_id,fecha_ingreso,precio_venta)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,date('now'),?)''',
                (body.get('marca',''), body.get('modelo',''), body.get('chasis',''),
                 ...
                 body.get('anio',''), body.get('color',''), body.get('compra_id'), body.get('precio_venta',0)))
            conn.commit()
            new_id = cur.lastrowid
            conn.close()
            self.send_json({'ok': True, 'id': new_id})
            ...
            for row in rows:
                ...
                modelo = re.search(r'MODELO:(.+?)(?:AÃO:|MOTOR:|CHASIS:|COLOR:|$)', det)
    ...
    # ââ PUT ââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    def do_PUT(self):
        path = urlparse(self.path).path
        usr  = check_auth(self)
        if not usr:
            ...
            if tabla == 'compras':
                ...
                conn.execute(
                    '''UPDATE inventario_stock SET marca=?,modelo=?,chasis=?,motor=?,matricula=?,padron=?,
                       precio=?,tipo=?,anio=?,color=?,vendido=?,precio_venta=? WHERE id=?''',
                    (body.get('marca',''), body.get('modelo',''), body.get('chasis',''),
                     body.get('motor',''), body.get('matricula',''), body.get('padron',''),
                     body.get('precio',0), body.get('tipo',''), body.get('anio',''),
                     body.get('color',''), body.get('vendido',0), body.get('precio_venta',0), rid))
