...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/stats':
            ...
            q      = qs.get('q', [''])[0]
            if q:
                rows  = conn.execute("SELECT * FROM compras WHERE proveedor LIKE ? OR detalle LIKE ? OR comprobante LIKE ? ORDER BY fecha DESC LIMIT ? OFFSET ?", ('%'+q+'%', '%'+q+'%', '%'+q+'%', limit, offset)).fetchall()
                total = conn.execute("SELECT COUNT(*) FROM compras WHERE proveedor LIKE ? OR detalle LIKE ? OR comprobante LIKE ?", ('%'+q+'%', '%'+q+'%', '%'+q+'%')).fetchone()[0]
            else:
            ...
            q      = qs.get('q', [''])[0]
            if q:
                rows  = conn.execute("SELECT * FROM ventas WHERE cliente LIKE ? OR detalle LIKE ? ORDER BY fecha DESC LIMIT ? OFFSET ?", ('%'+q+'%', '%'+q+'%', limit, offset)).fetchall()
                total = conn.execute("SELECT COUNT(*) FROM ventas WHERE cliente LIKE ? OR detalle LIKE ?", ('%'+q+'%', '%'+q+'%')).fetchone()[0]
            else:
            ...
            q      = qs.get('q', [''])[0]
            if q:
                rows  = conn.execute("SELECT * FROM clientes WHERE nombre LIKE ? OR documento LIKE ? ORDER BY nombre LIMIT ? OFFSET ?", ('%'+q+'%', '%'+q+'%', limit, offset)).fetchall()
                total = conn.execute("SELECT COUNT(*) FROM clientes WHERE nombre LIKE ? OR documento LIKE ?", ('%'+q+'%', '%'+q+'%')).fetchone()[0]
            else:
    ...
    def do_DELETE(self):
        ...
        if len(parts) == 3 and parts[1] in ('compras', 'ventas', 'clientes'):
            conn.execute(f'DELETE FROM {parts[1]} WHERE id=?', (int(parts[2]),))
            conn.commit()
            conn.close()
            self.send_json({'ok': True})
        else:
