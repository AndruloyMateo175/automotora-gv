...
def _do_sync(get_db_fn):
    ...
    try:
        ...
        if 'MenuPrincipal' not in html and 'menu' not in html.lower():
            _last_result = {'ok': False, 'error': 'Login fallido en eFactura'}
            return
        # CSV ventas
        url_v = (EF_URL + '/ListadoComprobantesExcel/ListadoComprobantesExcel_config_csv.php'
                 '?script_case_init=9170&summary_export_columns=S&password=n&nm_res_cons=n'
                 '&nm_ini_csv_res=grid&nm_all_modules=grid&nm_delim_line=1&nm_delim_col=1'
                 '&nm_delim_dados=1&nm_label_csv=N&language=es&origem=cons')
        url_c = (EF_URL + '/ListadoFacturasCompra/ListadoFacturasCompra_config_csv.php'
                 '?script_case_init=9170&summary_export_columns=S&password=n&nm_res_cons=n'
                 '&nm_ini_csv_res=grid&nm_all_modules=grid&nm_delim_line=1&nm_delim_col=1'
                 '&nm_delim_dados=1&nm_label_csv=N&language=es&origem=cons')
...
def _parse_ventas(text, conn):
    ...
    for row in list(csv.reader(io.StringIO(text), delimiter=';'))[1:]:
        ...
        if tipo not in ('e-Ticket', 'e-Factura'):
            continue
        comp = tipo + ' ' + row[1].strip().strip('"')
        if conn.execute('SELECT id FROM ventas WHERE comprobante=?', (comp,)).fetchone():
            continue
        ...
        try:
            precio = float(row[7].strip().strip('"').replace('.', '').replace(',', '.'))
        except Exception:
            precio = 0
...
def _parse_compras(text, conn):
    ...
    for row in list(csv.reader(io.StringIO(text), delimiter=';'))[1:]:
        if len(row) < 8:
            continue
        tipo = row[0].strip().strip('"')
        if tipo not in ('e-Ticket', 'e-Factura', 'e-Remito'):
            continue
        comp = tipo + ' ' + row[1].strip().strip('"')
        prov = row[4].strip().strip('"') if len(row) > 4 else ''
        if not any(v in prov.upper() for v in PROVEEDORES):
            continue
        if conn.execute('SELECT id FROM compras WHERE comprobante=?', (comp,)).fetchone():
            continue
        fecha = row[2].strip().strip('"')
        det = row[5].strip().strip('"') if len(row) > 5 else ''
        try:
